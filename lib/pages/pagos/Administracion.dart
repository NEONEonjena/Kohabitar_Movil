...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const PagosScreen(), // 👈 pantalla inicial
    );
  }
}
  ...
  return Container(
    ...
    child: Column(
      children: [
        ...
        ElevatedButton(
          onPressed: () {
            print("Botón presionado: $title"); // 👈 confirma que entra aquí
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => HistorialPagosScreen(title: title),
              ),
            );
          },
...
class HistorialPagosScreen extends StatelessWidget {
  ...
  const HistorialPagosScreen({super.key, required this.title});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Historial de $title"), // 👈 dinámico
        backgroundColor: const Color(0xFF26A69A),
      ),
      body: Center(
        child: Text(
          "Aquí se mostrará el historial de $title",
